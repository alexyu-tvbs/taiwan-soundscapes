...
**The Problem**: HAR files contain URLs for the recording environment (e.g., `dev.example.com`).
Playing back on a different environment fails.
...
When in playback mode, the Network Recorder automatically analyzes your HAR file to detect CRUD
patterns. If it finds:
...
**Tip**: We recommend setting `process.env.PW_NET_MODE` directly in your test file for better
control.
...
The recorder includes built-in file locking for safe parallel execution. Each test gets its own HAR
file based on the test name.
