...
use everywhere. **Works for both API-only tests and browser tests.**
...
**Context**: Test microservice authentication patterns (API keys, service tokens) with proper
...
> **Note**: API keys are typically static secrets that don't expire, so disk persistence
> (auth-session) isn't needed. For rotating service tokens, use the auth-session provider pattern
> from Example 6.
