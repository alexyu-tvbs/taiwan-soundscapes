...
- `POST /api/v2/case-management/cases` -> Pattern: `POST:500:/api/v2/case-management` (different
  group!)
...
The errors might be happening during page load or in background polling. Check the
`network-errors.json` artifact in your test report for full details including timestamps.
