...
Text extraction may fail for PDFs that store text as vector graphics (e.g., those generated by
jsPDF):
...
> **Note**: When `mergePages: false`, `content` is an array of strings (one per page). When
> `maxPages` is set, only that many pages are extracted.
